/bin/bash -lc cat > /tmp/matadd.c << 'EOF'
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
#include <time.h>
...
gcc -O3 -march=native -funroll-loops -pthread /tmp/matadd.c -o /tmp/matadd || gcc -O3 -pthread /tmp/matadd.c -o /tmp/matadd
...
CORES=$(nproc)
MAXT=$(( CORES*2 ))
if [ $MAXT -gt 32 ]; then MAXT=32; fi
THREADS="1 2 4 8 16 32"
# filter threads to <= MAXT
TARGS=""
for t in $THREADS; do if [ $t -le $MAXT ]; then TARGS="$TARGS $t"; fi; done
OUT=/tmp/results.csv
echo "N,threads,pattern,sec,checksum" > $OUT
for N in $SIZES; do
  for pat in $PATTERNS; do
    for t in $TARGS; do
      echo "Running N=$N pat=$pat threads=$t"
      /tmp/matadd $N $t $pat | tee /tmp/last_run.txt
      # extract CSV line
      L=$(grep '^CSV' /tmp/last_run.txt | sed 's/CSV,//')
      echo $L >> $OUT
    done
  done
done
